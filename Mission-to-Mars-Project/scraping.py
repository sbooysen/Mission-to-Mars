...
def scrape_all():
    ...
    img_url_titles = hem_img(browser)
    ...
    data = {
        ...
        "hemisphere_image_info": img_url_titles
    }
...
def featured_image(browser):
    ...
    try:
        ...
        img_url_rel = sample_img_soup.find('img', class_='fancybox-image').get('src')
...
def hem_img(browser):
    ...
    try:
        ...
        for hem in range(pics_count):
            # Create an empty dict to hold the search results
            results = {}
            # Find link to picture and open it
            link_image = main_page_soup.select("div.description a")[hem].get('href')
